...
classdef spatial_grid_symmetric < discretizations.spatial_grid
	...
	methods
        ...
        function objects = spatial_grid_symmetric( grids_elements, grid_FOV )
            ...
            %--------------------------------------------------------------
            FOV_pos_ctr = 2 * grid_FOV.offset_axis( 1:(end - 1) ) + ( grid_FOV.N_points_axis( 1:(end - 1) ) - 1 ) .* grid_FOV.cell_ref.edge_lengths( 1:(end - 1) );
            ...
            %--------------------------------------------------------------
            must_equal_integer = ( grids_elements( 2 ).positions( 1 ) - grids_elements( 1 ).positions( 1 ) ) ./ grid_FOV.cell_ref.edge_lengths( 1:(end - 1) );
