...
% TODO: move to different class
	...
	methods
        ...
        function intervals_tof = times_of_flight( setups, varargin )
            ...
            indicator_FOV = cellfun( @( x ) isa( x, 'fields_of_view.orthotope' ), { setups.FOV } );
            if ~( all( indicator_array_planar( : ) ) && all( indicator_FOV( : ) ) )
...
% TODO: does lateral extent of FOV contain face_ctr?
%                         if intersection( setups( index_setup ).FOV.intervals, face_ctr_intervals )
                            ...
                            %----------------------------------------------
                            t_tof_lbs( index_active_tx, index_active_rx ) = 2 * sqrt( dist_focus_ctr^2 + setups( index_setup ).FOV.intervals( end ).lb^2 ) / setups( index_setup ).homogeneous_fluid.c_avg;
                            ...
                            [ dist_ctr_vertices_max_lb, index_max_lb ] = max( vecnorm( [ [ face_ctr.intervals.lb ], 0 ] - pos_vertices{ index_setup }, 2, 2 ) );
                            [ dist_ctr_vertices_max_ub, index_max_ub ] = max( vecnorm( [ [ face_ctr.intervals.ub ], 0 ] - pos_vertices{ index_setup }, 2, 2 ) );
...
% TODO: check symmetry of setups and choose class accordingly
            if 1 %issymmetric( discretizations_elements, discretization_FOV )
                N_points_per_pitch_axis = round( setups.xdc_array.cell_ref.edge_lengths ./ options_spatial.options_FOV.values( 1:(end - 1) ) );
