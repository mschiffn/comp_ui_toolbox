...
% modified: 2020-01-10
%
classdef tx_rnd_apo_del < scattering.sequences.settings.controls.tx
	...
	methods
        ...
        % constructor
        %------------------------------------------------------------------
        function objects = tx_rnd_apo_del( setup, u_tx_tilde, e_theta, settings_rng_apo, settings_rng_del )
            ...
            if ~( isa( setup, 'scattering.sequences.setups.setup' ) && isscalar( setup ) )
                ...
                errorStruct.identifier = 'tx_rnd_apo_del:NoSetup';
                error( errorStruct );
            end
            ...
            if ~isa( setup.xdc_array, 'scattering.sequences.setups.transducers.array_planar_regular_orthogonal' )
                ...
                errorStruct.identifier = 'tx_rnd_apo_del:NoRegularOrthogonalTransducerArray';
                error( errorStruct );
            end
            ...
            if ~isa( e_theta, 'math.unit_vector' )
                ...
                errorStruct.identifier = 'tx_rnd_apo_del:NoUnitVector';
                error( errorStruct );
            end
            ...
            if ~( isa( settings_rng_apo, 'auxiliary.setting_rng' ) && isa( settings_rng_del, 'auxiliary.setting_rng' ) )
                ...
                errorStruct.identifier = 'tx_rnd_apo_del:NoSettingRng';
                error( errorStruct );
            end
            ...
            for index_object = 1:N_objects
                ...
                impulse_responses{ index_object } = processing.delta_matrix( indices_q( : ), setup.T_clk, physical_values.meter_per_volt_second( apodization_weights_act ) );
        ...
        end % function objects = tx_rnd_apo_del( setup, u_tx_tilde, e_theta, settings_rng_apo, settings_rng_del )

	end % methods

end % classdef tx_rnd_apo_del < scattering.sequences.settings.controls.tx
