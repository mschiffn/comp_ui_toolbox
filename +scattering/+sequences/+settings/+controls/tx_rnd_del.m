...
% modified: 2019-11-28
%
classdef tx_rnd_del < scattering.sequences.settings.controls.tx
	...
	methods
        ...
        % constructor
        %------------------------------------------------------------------
        function objects = tx_rnd_del( setup, u_tx_tilde, e_theta, settings_rng )
            ...
            if ~( isa( setup, 'scattering.sequences.setups.setup' ) && isscalar( setup ) )
                ...
                errorStruct.identifier = 'tx_rnd_del:NoSetup';
                error( errorStruct );
            end
            ...
            if ~isa( setup.xdc_array, 'scattering.sequences.setups.transducers.array_planar_regular_orthogonal' )
                ...
                errorStruct.identifier = 'tx_rnd_del:NoRegularOrthogonalTransducerArray';
                error( errorStruct );
            end
            ...
            if ~isa( e_theta, 'math.unit_vector' )
                ...
                errorStruct.identifier = 'tx_rnd_del:NoUnitVector';
                error( errorStruct );
            end
            ...
            if ~isa( settings_rng, 'auxiliary.setting_rng' )
                ...
                errorStruct.identifier = 'tx_rnd_del:NoSettingRng';
                error( errorStruct );
            end
...
%                 T_inc = physical_values.second( 2e-6 );	% test using full pulse length
        ...
        end % function objects = tx_rnd_del( setup, u_tx_tilde, e_theta, settings_rng )

	end % methods

end % classdef tx_rnd_del < scattering.sequences.settings.controls.tx
