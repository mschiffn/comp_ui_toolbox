...
classdef array
	...
	properties (SetAccess = private)
        ...
        aperture ( :, 1 ) scattering.sequences.setups.transducers.face { mustBeNonempty } = scattering.sequences.setups.transducers.face % aperture is a column vector of vibrating faces
	...
	methods
        ...
        function objects = array( apertures )
            ...
            for index_object = 1:numel( objects )
                ...
                % ensure class scattering.sequences.setups.transducers.face (column vector)
                if ~( isa( apertures{ index_object }, 'scattering.sequences.setups.transducers.face' ) && iscolumn( apertures{ index_object } ) )
                    errorStruct.message = sprintf( 'apertures{ %d } must be a column vector of scattering.sequences.setups.transducers.face!', index_object );
...
% TODO: prevent overlapping of faces
        ...
        function arrays = discretize( arrays, methods_faces )
            ...
            if ~isa( arrays, 'scattering.sequences.setups.transducers.array' )
                errorStruct.message = 'arrays must be scattering.sequences.setups.transducers.array!';
