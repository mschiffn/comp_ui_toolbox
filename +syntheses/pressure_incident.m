...
classdef pressure_incident < syntheses.field
    ...
    methods
        ...
        % constructor
        %------------------------------------------------------------------
        function objects = pressure_incident( setup, measurements )

            %--------------------------------------------------------------
            % 1.) constructor of superclass
            %--------------------------------------------------------------
            objects@syntheses.field( setup, measurements );
            ...
            for index_object = 1:N_objects
                ...
                %----------------------------------------------------------
                % create filename
                str_filename = sprintf( '%s_setup_%s/p_in_measurement_%s.mat', setup.str_name, str_hash_setup, hash( measurements( index_object ) ) );
                ...
                if indicator_file_exists
                    ...
                    if ~isequal( temp.setup, setup ) || ~isequal( temp.measurement_act, measurements( index_object ) )
                ...
                if indicator_pressure_exists
                    ...
                    if isa( setup, 'pulse_echo_measurements.setup_symmetric' )
                        %
                        objects( index_object ) = compute_p_in_symmetric( objects( index_object ), setup, measurements( index_object ) );
                    else
                        %
                        objects( index_object ) = compute_p_in_symmetric( objects( index_object ), setup, measurements( index_object ) );
                    end
                    ...
                    %------------------------------------------------------
                    N_elements_active = numel( measurements( index_object ).settings_tx.indices_active );
                    if N_elements_active >= 2
                        pressure_incident_act = objects( index_object );
                        measurement_act = measurements( index_object );
                        if indicator_file_exists
        ...
        end % function objects = pressure_incident( setup, measurements )

        %------------------------------------------------------------------
        % compute incident acoustic pressure field (symmetric grids)
        %------------------------------------------------------------------
        function object = compute_p_in_symmetric( object, setup, measurement )
            ...
            %--------------------------------------------------------------
            N_elements_active = numel( measurement.settings_tx.indices_active );
